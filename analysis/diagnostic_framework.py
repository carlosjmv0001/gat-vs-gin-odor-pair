...
# Setup para carregamento de modelos  
...
class DiagnosticFramework:  
    """Framework unificado para análise diagnóstica de GIN vs GAT"""  
      
    def __init__(self):  
        # Marcadores estruturais expandidos  
        ...
        # Conceitos agregados expandidos  
    ...
    def load_models_and_data(self):  
        """Carrega modelos e dados de forma consistente com outras análises"""  
    ...
    def get_predictions_and_labels(self, model, dataset, device):  
        """Obtém predições e rótulos - padrão consistente"""  
    ...
    def create_high_impact_table(self, results_file):  
        """Cria tabela de alto impacto com métricas solicitadas"""  
        ...
        for i, note in enumerate(results['class_names']):  
            ...
            # Classificação expandida - VERSÃO CORRIGIDA  
            if note in self.structural_markers:  
                ...
                note_type = 'unknown'  # Mantém como fallback  
    ...
    def analyze_inductive_bias(self, df):  
        """Analisa padrões de inductive bias"""  
        # Análise estatística  
        delta_stats = {  
    ...
    def design_conditional_strategy(self, bias_analysis):  
        """Projeta estratégia de seleção condicional"""  
        GAT_THRESHOLD = 0.05  
        GIN_THRESHOLD = -0.05  
    ...
    def simulate_conditional_performance(self, df, strategy):  
        """Simula performance da estratégia condicional"""  
        conditional_aurocs = []  
    ...
    def create_visualizations(self, df, bias_analysis, results_dir):  
        """Cria visualizações diagnósticas"""  
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))  
          
        # 1. Delta plot por tipo  
        ...
        # 2. High impact table visualization - MODIFICADO PARA TOP 5  
        high_impact = bias_analysis['high_impact_notes'].head(5)  # MUDADO AQUI  
        ...
        ax2.set_title('Top 5 High-Impact Notes')  # MUDADO AQUI  
    ...
    def run_complete_analysis(self):  
        """Executa análise diagnóstica completa"""  
        ...
        if not os.path.exists(results_file):  
            print("Execute detailed_performance_analysis.py primeiro!")  
            return  
          
        # 1. Criar tabela de alto impacto  
        df = self.create_high_impact_table(results_file)  
          
        # 2. Analisar inductive bias  
        bias_analysis = self.analyze_inductive_bias(df)  
          
        # 3. Projetar estratégia condicional  
        strategy = self.design_conditional_strategy(bias_analysis)  
          
        # 4. Simular performance  
        ...
        # Imprimir resumo  
        print(f"\n=== RESUMO DA ANÁLISE DIAGNÓSTICA ===")  
        print(f"Notas analisadas: {len(df)}")  
        print(f"GAT-dominadas: {len(bias_analysis['gat_dominated_notes'])}")  
        print(f"GIN-dominadas: {len(bias_analysis['gin_dominated_notes'])}")  
        print(f"Melhoria esperada com estratégia condicional: {conditional_results['improvement_over_gin']:.4f}")  
