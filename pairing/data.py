...
CUTOFF = 1e3 # 33 classes with this cutoff
# CUTOFF = 100 # 74 classes with this cutoff
    ...
    for data in usable:
        ...
        for (other, note) in data["blenders"]:
            ...
            if note is None or (isinstance(note, str) and note.strip() == ""):
                continue
            if note == 'No flavor group found for these' or note == 'No odor group found for these':
                continue
            if data["smiles"] == name_to_smiles[other]:
                self_edges += 1
                continue
            std_note = standardize_note(note)
            if not std_note or std_note == '':
                continue
            pair = order_pair(data["smiles"], name_to_smiles[other])
            pairings[pair][std_note] += 1
            if not std_note in note_counts:
                note_counts[std_note] = 0
            note_counts[std_note] += 1
...
class Dataset(InMemoryDataset):  
    def __init__(self, is_train):  
        ...
        torch.serialization.add_safe_globals([sys.modules['__main__'].PairData])  
        if is_train:  
    ...
    @classmethod
    def num_classes(cls):
        with open(os.path.join(out_dir, notes_fname), "r") as f:
            notes = f.read().split(",")
